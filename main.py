...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        label { display: block; margin-bottom: 8px; font-size: 12px; font-weight: 700; color: var(--text-gray); text-transform: uppercase; letter-spacing: 1px; }
        .input-group { position: relative; margin-bottom: 25px; }
        .input-group i { position: absolute; left: 16px; top: 50%; transform: translateY(-50%); color: #666; }
        input, select { width: 100%; padding: 16px 16px 16px 45px; background: var(--input); border: 1px solid #333; border-radius: 12px; color: white; font-size: 15px; font-family: inherit; transition: all 0.2s; outline: none; }
        select { padding-left: 16px; appearance: none; cursor: pointer; }
        input:focus, select:focus { border-color: var(--accent); box-shadow: 0 0 0 4px rgba(255, 0, 51, 0.1); }
        button { width: 100%; padding: 18px; background: var(--accent); color: white; border: none; border-radius: 12px; font-size: 16px; font-weight: 700; cursor: pointer; transition: all 0.3s; box-shadow: 0 10px 20px -5px rgba(255, 0, 51, 0.4); }
        button:hover { transform: translateY(-2px); box-shadow: 0 15px 25px -5px rgba(255, 0, 51, 0.5); filter: brightness(110%); }
        /* Loading Overlay */
        #loader { display: none; position: absolute; inset: 0; background: rgba(15, 15, 15, 0.95); border-radius: 24px; z-index: 100; flex-direction: column; justify-content: center; align-items: center; text-align: center; }
...
def convert():
    ...
    # CHECK FOR COOKIES FILE to bypass "Sign in to confirm you're not a bot"
    cookie_args = {}
    ...
    try:
        ...
        # 2. Convert using FFmpeg logic
        ffmpeg_cmd = ['ffmpeg', '-y', '-i', temp_wav]

        # --- EXTENSIVE FORMAT LOGIC ---
        if fmt == 'mp3':
        ...
        # Telephony / Retro (Downsample to 8000Hz to prevent crashes)
        elif fmt == 'amr':
        ...
        elif fmt in ['flac', 'wav', 'alac', 'aiff', 'ape', 'wv', 'tta']: 
            pass 
        ...
        <body style="background:#0f0f0f; color:#fff; font-family:sans-serif; display:flex; justify-content:center; align-items:center; height:100vh; text-align:center;">
            <div style="background:#1c1c1c; padding:40px; border-radius:20px; border:1px solid #333; max-width:500px;">
                ...
                {'<div style="background:#332200; color:#ffaa00; padding:15px; border-radius:10px; font-size:13px; margin-bottom:20px;"><strong>TIP:</strong> You need to upload your cookies.txt to GitHub to fix the "Sign in" error.</div>' if 'Sign in' in error_msg else ''}
