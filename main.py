...
def convert():
    ...
    # --- ⚠️ COOKIES REMOVED ---
    # We are using the "TV Embedded" strategy. 
    # This strategy works BEST without any cookies.
    # If we use your browser cookies on a server IP, YouTube detects the mismatch.
    # By being a "TV", we avoid the captcha check entirely.
    ...
    ydl_opts = {
        ...
        # 👇 THE FIX: "TV Mode" (No Cookies Needed) 👇
        'extractor_args': {
            'youtube': {
    ...
    try:
        ...
        <body style="background:#0f0f0f; color:#fff; font-family:sans-serif; display:flex; justify-content:center; align-items:center; height:100vh; text-align:center;">
            <div style="background:#1c1c1c; padding:40px; border-radius:20px; border:1px solid #333; max-width:500px;">
                ...
                <p style="color:#ccc; margin-bottom:20px; font-size: 14px;">{error_msg}</p>
                <div style="background:#222; padding:10px; border-radius:5px; font-size:12px; color:#888; text-align:left;">
                    <strong>Note:</strong> We tried to access YouTube as a TV client to bypass restrictions. If this fails, the server IP is completely blocked.
                </div>
                <br>
