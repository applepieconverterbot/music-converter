...
def convert():
    ...
    # 🚀 NEW SERVER LIST (Fresh Mirrors)
    api_servers = [
        "https://co.wuk.sh/api/json",            # Tier 1
        "https://api.cobalt.tools/api/json",     # Tier 1 (Official)
        "https://cobalt.gamers.taipei/api/json", # Tier 2
        "https://phobos.api.sc/api/json",        # Tier 2
        "https://cobalt.q1.pm/api/json",         # Tier 3
        "https://cobalt.synbay.app/api/json"     # Tier 3
    ]
    
    # Randomize list slightly to prevent hammering the same server if multiple people use your site
    random.shuffle(api_servers)

    # 🛑 STEALTH HEADERS (Pretending to be Google Chrome)
    # This stops Cloudflare from blocking the request
    headers = {
    ...
    for api_url in api_servers:
        try:
            ...
            try:
                ...
                # If server returns HTML instead of JSON (error page), skip it
                continue
            ...
            if response.status_code == 200 and 'url' in data:
                ...
                # If it's an "Invalid Link" error, don't try other servers (user fault)
    ...
    <body style="background:#0f0f0f; color:#fff; font-family:sans-serif; text-align:center; padding:50px;">
        ...
        <p>We tried {len(api_servers)} remote servers, but all are busy or blocked.</p>
        <p style="color:#888; font-size:12px;">Last specific error: {last_error}</p>
